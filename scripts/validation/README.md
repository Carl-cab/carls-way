...
| --- | --- | --- |
| `bcrypt_dummy_hash_timing_probe.mjs` | Measures a valid bcrypt comparison against an intentionally malformed dummy hash to demonstrate why a valid precomputed dummy hash is required for login timing normalization. | Uses example strings only; no database or network access. |
| `unsafe_postgres_numeric_parser_probe.mjs` | Demonstrates why globally parsing PostgreSQL `NUMERIC` values with JavaScript `parseFloat` is unsafe, especially for high-precision FX rates. | Uses a local disposable `manna_test` table named `numeric_parser_probe`, then removes it. It deliberately demonstrates the unsafe parser and must never be copied into app configuration. |
