...
export class SandboxCAProvider implements TransferProvider {
  ...
  async reviewTransfer(intentId: number, userId: number): Promise<ReviewResult> {
    ...
    const consentLanguage = row.type === 'add_money'
      ? `By confirming, you authorize Manna to initiate a Canadian EFT debit from your ${row.institution_name} account ending in ${row.account_mask || 'XXXX'} for ${row.currency} ${Number(row.amount).toFixed(2)} and credit your Manna balance. Sandbox mode: no real bank funds move, but your Manna balance will update.`
      : `By confirming, you authorize Manna to initiate a Canadian EFT credit to your ${row.institution_name} account ending in ${row.account_mask || 'XXXX'} for ${row.currency} ${Number(row.amount).toFixed(2)} and debit your Manna balance. Sandbox mode: no real bank funds move, but your Manna balance will update.`;
  ...
  async confirmTransfer(intentId: number, userId: number): Promise<ConfirmResult> {
    ...
    const settlement = await settleSandboxTransfer(intentId, userId, 'sandbox_ca');
